...
contract UpCake  is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "UpCake";
    string constant _symbol = "UPC";
    ...
    function setIsDividendExempt(address holder, bool exempt) external authorized {
        ...
        if(exempt){
            ...
            distributor.setShare(holder, _rBalance[holder]);
        }
    }
...
function multiTransfer(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
    ...
    for(uint i=0; i < addresses.length; i++){
        ...
        if(!isDividendExempt[addresses[i]]) {
            try distributor.setShare(addresses[i], _rBalance[addresses[i]]) {} catch {} 
        }
    }

    // Dividend tracker
    if(!isDividendExempt[from]) {
        try distributor.setShare(from, _rBalance[from]) {} catch {}
    }
}
